#
# = Capistrano database.yml and smtp_settings.yml task
#
# Provides a couple of tasks for creating the database.yml
# configuration file dynamically when deploy:setup is run.
# Added tasks for generating an smtp_settings to externalize the SMTP password
#
# Category::    Capistrano
# Package::     Database
...
Capistrano::Configuration.instance.load do
  ...
  namespace :deploy do
    ...
    namespace :db do
      ...
      desc <<-DESC
        Creates a database.yml configuration file in the shared path.

        By default, this task uses an internal template unless a template \
        called database.yml.erb is found either in :template_dir \
        or the /config/deploy folders. The default template matches \
        the template of the config/database.yml file shipped with Rails.

        When this recipe is loaded, db:setup is automatically configured \
        to be invoked after deploy:setup. You can skip this task by setting \
      ...
      task :setup, :except => { :no_release => true } do
        ...
        production:
          adapter: postgresql
            ...
            database: soft-werker_xxx
            username: soft-werker_xxx
            password: xxxxxxx
            encoding: utf8
            pool: 5
            timeout: 5000
    ...
    namespace :smtp do
      ...
      desc <<-DESC
        Creates a smtp_settings.yml configuration file in the shared path.

        This task uses an internal template to setup a shared config \
        file

        This task is currently not setup to run at any time automatically. \
        Use it a your own choosing.
      DESC
      task :setup, :except => { :no_release => true } do
...
#        config = ERB.new(template)
        ...
        run "mkdir -p #{shared_path}/config"
        put template, "#{shared_path}/config/smtp_settings.yml"
      end
