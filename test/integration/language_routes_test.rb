...
describe 'Language Routes Integration Test' do
  ...
  it 'should be able to route set language' do
    ...
    assert_recognizes({ controller: "language", action: "set_language" }, {path: "/set_language", method: 'post'})
    assert_generates("/set_language", { controller: "language", action: "set_language" })
  end
end